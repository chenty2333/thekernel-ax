...
publish_dry_run "$axcbpf_toolchain" thekernel-axpmu
...
if [[ "${AXTASK_REGISTRY_READY:-0}" == 1 ]]; then
    ...
    printf 'publish-dry-run: PASS (5 packages)\n'
else
    printf '%s\n' \
        'publish-dry-run: original mechanisms and coordinated leaf packages PASS; thekernel-axtask is registry-blocked until both 0.1.0 sibling packages are published' \
